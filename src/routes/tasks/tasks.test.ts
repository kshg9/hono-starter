...
import env from "@/env";
import { ZOD_ERROR_CODES, ZOD_ERROR_MESSAGE } from "@/lib/constants";
...
describe("tasks routes", () => {
  ...
  it("post /tasks validates the body when creating", async () => {
    ...
    if (response.status === 422) {
      ...
      expect(json.error.issues[0].message).toBe(ZOD_ERROR_MESSAGE.REQUIRED);
    }
  });
  ...
  it("get /tasks/{id} validates the id param", async () => {
    ...
    if (response.status === 422) {
      ...
      expect(json.error.issues[0].message).toBe(ZOD_ERROR_MESSAGE.EXPECTED_NUMBER);
    }
  });
  ...
  it("patch /tasks/{id} validates empty body", async () => {
    ...
    if (response.status === 422) {
      ...
      expect(json.error.issues[0].message).toBe(ZOD_ERROR_MESSAGE.NO_UPDATES);
    }
  });
